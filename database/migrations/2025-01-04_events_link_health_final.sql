-- Events Link Health Migration for TaxProExchange (FINAL CORRECTED)
...
DROP POLICY IF EXISTS staging_events_read_all ON staging_events;
DROP POLICY IF EXISTS staging_events_admin_write ON staging_events;
DROP POLICY IF EXISTS event_url_tombstones_read_all ON event_url_tombstones;
DROP POLICY IF EXISTS event_url_tombstones_admin_write ON event_url_tombstones;
