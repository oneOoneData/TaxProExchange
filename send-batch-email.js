  ...
  function sendOne(index) {
    ...
    var req = https.request({
      ...
      res.on("end", function() {
        ...
        if (result.id) {
          sent++;
          console.log("✅ " + (index+1) + "/" + recipients.length + " " + r.name + " <" + r.email + ">");
        } else {
          errors++;
          console.log("❌ " + (index+1) + "/" + recipients.length + " " + r.name + " - " + JSON.stringify(result));
        }
        // Next with small delay to avoid rate limits
        setTimeout(function() { sendOne(index + 1); }, 200);
      });
    });
